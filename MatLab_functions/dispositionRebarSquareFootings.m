...
for i=1:nv1
    nvi=nvi+1;
    sumaDiam=sumaDiam+rebarAvailable(RebarArrangement1(nvi),3);
end
...
for i=1:nv2
    nvi=nvi+1;
    sumaDiam=sumaDiam+rebarAvailable(RebarArrangement2(nvi),3);
end
...
dispositionRebar(1,2)=-hprima*0.5+(rebarAvailable(7,3)*0.5)+dextra;
...
dispositionRebar(nv1+1,2)=hprima*0.5-(rebarAvailable(7,3)*0.5)-dextra*0.5;
...
for j=2:nv1
    nivar_tension=nivar_tension+1;
    xacumInf=xacumInf+0.5*rebarAvailable(RebarArrangement1(nivar_tension-1),3)+...
        separacion_b_inf+0.5*rebarAvailable(RebarArrangement1(nivar_tension),3);

    dispositionRebar(nivar_tension,1)=xacumInf;
    dispositionRebar(nivar_tension,2)=-hprima*0.5+(rebarAvailable(7,3)*0.5)+dextra; 
end

% compression
for j=2:nv2
    nivar_comp=nivar_comp+1;
    xacumSup=xacumSup+0.5*rebarAvailable(RebarArrangement2(nivar_comp-1),3)+separation_b_sup+...
        0.5*rebarAvailable(RebarArrangement2(nivar_comp),3);

    dispositionRebar(nv1+nivar_comp,1)=xacumSup;
    dispositionRebar(nv1+nivar_comp,2)=hprima*0.5-rebarAvailable(7,3)*0.5-dextra*0.5;
end
