...
c_vector=zeros(npuntos,2);
diagramaInteraccion=zeros(npuntos,9);
...
pot=act*fy*0.001;
df=(poc+pot)/(npuntos-1);
dfi=0.1/(npuntos-1);
c_vector(1,:)=4*h;

diagramaInteraccion(1,1)=-poc;
diagramaInteraccion(npuntos,1)=pot;
...
for sentido=1:2
    ...
    diagramaInteraccion(npuntos,4*sentido-1)=0.75*pot;
    
    diagramaInteraccion(npuntos,4*sentido)=0;
    diagramaInteraccion(npuntos,2)=0;
    diagramaInteraccion(npuntos,6)=0;
end

%%%%%%%%%%%%%%%%%%%%%%%%%%% Eficiencias %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

[maxef,tablaEficiencias,cxy]=effRecColsBinarySearch(diagramaInteraccion,...
