...
%                               size = [7,5] by default, in format: 
%               _______________________________________________
%               [noption,# rebar,diam,rebar-area,lineal-weight]
%               -----------------------------------------------
%
...
if largerDim==1
    acprima=nvt(1)*rebarAvailable(arrangement1(1),4)*(2*bz/(lz+bz));
    lextremos=(lz-bz)*0.5;
    bzp=bz;
elseif largerDim==2
    
    acprima=nvt(1)*rebarAvailable(arrangement1(1),4)*(2*lz/(lz+bz));
    lextremos=(bz-lz)*0.5;
    bzp=lz;
end
...
while av_red<acprima
    nv_red=nv_red+1;
    av_red=av_red+rebarAvailable(arrangement1(1),4);
end
...
ac_rest=(nvt(1)-nv_red)*rebarAvailable(arrangement1(1),4);
...
for i=1:nv1
    nvi=nvi+1;
    sumaDiam=sumaDiam+rebarAvailable(arrangement1(nvi),3);
end
...
nivar_tension=0;
xacumInf=-bprima*0.5+0.5*rebarAvailable(7,3);
disposicion_varillado(1,1)=xacumInf;
disposicion_varillado(1,2)=-hprima*0.5+(rebarAvailable(7,3)*0.5)+dextra;
...
for j=2:nv1
    nivar_tension=nivar_tension+1;
    xacumInf=xacumInf+0.5*rebarAvailable(arrangement1(nivar_tension-1),3)+...
        separacion_b_inf+0.5*rebarAvailable(arrangement1(nivar_tension),3);

    disposicion_varillado(nivar_tension,1)=xacumInf;
    disposicion_varillado(nivar_tension,2)=-hprima*0.5+(rebarAvailable(7,3)*0.5)+dextra;
end
...
for i=1:nv2
    nvi=nvi+1;
    sumaDiam=sumaDiam+rebarAvailable(arrangement2(nvi),3);
end
separacion_b_sup=((bprima-sumaDiam)/(nv2-1));

% initial values of rebar coordinates in compression
nivar_comp=0;
xacumSup=-bprima*0.5+0.5*rebarAvailable(7,3);
disposicion_varillado(nv1+1,1)=xacumSup;
disposicion_varillado(nv1+1,2)=hprima*0.5-(rebarAvailable(7,3)*0.5)-dextra*0.5;
...
for j=2:nv2
    nivar_comp=nivar_comp+1;
    xacumSup=xacumSup+0.5*rebarAvailable(arrangement2(nivar_comp-1),3)+separacion_b_sup+...
        0.5*rebarAvailable(arrangement2(nivar_comp),3);

    disposicion_varillado(nv1+nivar_comp,1)=xacumSup;
    disposicion_varillado(nv1+nivar_comp,2)=hprima*0.5-rebarAvailable(7,3)*0.5-dextra*0.5;
...
sumaDiam=nv_ends*rebarAvailable(arrangement1(1),3);
tma=0.75; %pulg
sepMin=3/2*tma*2.54;
...
if nv_ends~=0
    ...
    disposicion_varillado(nv_red+1,2)=-hprima*0.5+(rebarAvailable(7,3)*0.5)+dextra;
    ...
    for j=2:npacs_ext
        ...
        nivar_tension=nivar_tension+1;
        xacumInf=xacumInf+0.5*rebarAvailable(arrangement1(nivar_tension-1),3)+...
            separacion_ext+0.5*rebarAvailable(arrangement1(nivar_tension),3);

        disposicion_varillado(nv_red+nivar_tension,1)=xacumInf;
        disposicion_varillado(nv_red+nivar_tension,2)=-hprima*0.5+(rebarAvailable(7,3)*0.5)+dextra;
    ...
    disposicion_varillado(nv_red+nivar_tension+1,2)=-hprima*0.5+(rebarAvailable(7,3)*0.5)+dextra;
